...
Scanning is cheap: the full marketplace lints in under a second ($\approx$9\,ms per definition).
The full artifact is versioned with a public DOI and contains the code, paper source, benchmark,
metamorphic tests, baseline scripts, and release-gate checks needed to reproduce the headline
claims. The core claim is narrow: privilege risk in agent definitions can be a deterministic,
...
\begin{itemize}\setlength\itemsep{2pt}
  ...
  \item A public reproducibility package with a citable DOI, release artifacts, source paper,
    benchmark scripts, CI gates, and explicit artifact commands, so the paper can be inspected as a
    runnable system rather than a standalone claim.
\end{itemize}
...
\label{sec:repro}
The artifact is public and versioned: the repository release contains the rendered paper and arXiv
source archive, and the Zenodo concept DOI is
\href{https://doi.org/10.5281/zenodo.21420490}{10.5281/zenodo.21420490}. The paper's claims are
designed to be checked without credentials, paid model calls, or private data. The minimum
...
python -m pytest -q
\end{verbatim}
The full release gate additionally runs type checking, contract verification, workflow audit,
package-build validation, and AgentGuard's own publish check. This matters for the central claim:
the linter is not evaluated by a one-off notebook but by the same deterministic gate a user would
run in CI. The artifact intentionally separates three classes of evidence: frozen benchmark cases
for recall/precision, mechanically generated metamorphic variants for stability, and field-study
snapshots for ecosystem prevalence. No claim in this paper depends on a private corpus or
non-reproducible model judgment.
...
The strongest possible negative result would be an independently labeled corpus where the AL300
chain condition produces many false alarms once human reviewers inspect the definitions. That would
not merely lower a metric; it would falsify the paper's central engineering claim that the
structural precondition is precise enough to gate by default. The current evidence argues against
that outcome in three ways: zero false alarms on the gated negative set, precision-hardening from
real marketplace false positives, and a manual precision check over the independent third-party
chain findings. Still, the next step is external human labeling, not another self-authored
benchmark expansion.
...
AgentGuard scans public or user-provided agent definitions for structural security risk. The field
study reports aggregate exposure rates and does not publish a per-author vulnerability list or
claim that specific packages were exploited. The worked exploit example is synthetic and
minimal; it is included to make the capability chain concrete without targeting a live system.
The artifact contains deterministic scripts and generated reports; no private logs, credentials,
or personal data are required to reproduce the results. AI assistance was used for drafting,
editing, and local iteration of the manuscript and artifact text; all reported measurements come
from committed scripts or explicitly described manual checks.
